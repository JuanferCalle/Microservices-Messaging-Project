...
SERVICE_NAME = os.getenv("SERVICE_NAME", "admission")
...
def publish_message(message: dict, routing_key: str = "patient.arrived"):
    ...
    try:
        ...
        pid = None
    print(f"Admission published routing_key={routing_key} patient_id={pid}")
    conn.close()
...
@app.get("/")
async def root():
    return {"service": SERVICE_NAME, "message": "Admission service is running"}


# Pre-seeded patients for admission
PRESEED = [
...
async def simulate_arrival(payload: dict, background: BackgroundTasks):
    # Publish single arrival event
    ...
    try:
        ...
        pid = None
    print(f"simulate_arrival: queued publish patient.arrived for patient_id={pid}")
